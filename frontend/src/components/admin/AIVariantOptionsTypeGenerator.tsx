/**
 * AI Variant Option Types Generator Component
 *
 * Generates variant option types (like Size, Color, Material)
 * using AI based on product category
 */
...
import { generateVariantOptions, type GeneratedVariantOptionType } from '../../api/ai';

export interface AIVariantOptionsTypeGeneratorProps {
  existingOptions: string[]; // Option names that already exist
  onOptionsGenerated: (options: GeneratedVariantOptionType[]) => void;
}

export const AIVariantOptionsTypeGenerator: React.FC<AIVariantOptionsTypeGeneratorProps> = ({
  existingOptions,
  onOptionsGenerated
}) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      console.log('🎨 Generating variant option types with AI...');

      const result = await generateVariantOptions({
        productCategory: formData.productCategory.trim() || undefined,
        productType: formData.productType.trim() || undefined,
        numberOfOptions: formData.numberOfOptions,
        existingOptions
      });

      console.log(`✅ Generated ${result.options.length} variant option types`);
      ...
      console.error('❌ Error generating variant option types:', err);
      setError(err.response?.data?.error || err.message || 'Failed to generate variant option types');
    } finally {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-display text-2xl uppercase tracking-wider text-champagne">
                      AI Option Types Generator
                    </h2>
                    <p className="text-sm text-champagne/60">
                      Generate industry-standard variant option types
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="mb-6 rounded-lg border border-purple-500/20 bg-purple-500/10 p-4 flex gap-3">
                ...
                <div className="text-sm text-champagne">
                  ...
                  <p className="text-champagne/70">
                    AI analyzes your product category and generates appropriate variant option types
                    (like Size, Color, Material) that customers use to filter and select products.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium text-champagne/80">
                    Product Category (optional but recommended)
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-champagne/60">
                    More specific = more relevant option types
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, numberOfOptions: parseInt(e.target.value) || 5 })}
                  ...
                  />
                  <p className="mt-1 text-xs text-champagne/60">
                    Range: 3-10. Start with 4-5 for most categories.
                  </p>
                </div>
                ...
                {existingOptions.length > 0 && (
                  <div className="rounded-lg border border-white/10 bg-white/5 p-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {existingOptions.slice(0, 10).map((opt) => (
                        <span key={opt} className="rounded-md bg-white/5 px-2 py-0.5 text-xs text-champagne/60">
                          {opt}
                        </span>
                      ))}
                      {existingOptions.length > 10 && (
                        <span className="rounded-md bg-white/5 px-2 py-0.5 text-xs text-champagne/60">
                          +{existingOptions.length - 10} more
                        </span>
                      )}
                    </div>
                    <p className="mt-2 text-xs text-champagne/60">
                      AI will avoid duplicating these option types
                    </p>
                  </div>
                )}
                ...
                {success && (
                  <div className="rounded-lg border border-green-500/20 bg-green-500/10 p-4 flex gap-3">
                    ...
                    <div className="text-sm text-champagne">
                      Option types generated successfully!
                    </div>
                  </div>
                )}
              </div>
...
export default AIVariantOptionsTypeGenerator;
